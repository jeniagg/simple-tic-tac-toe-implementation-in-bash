...
while  [ $(( $(grep "[a-zA-Z]" symbol.tmp| wc -m) - 1 )) -ne 1 ]
do
        echo "the symbol must be of one symbol [a-zA-Z] "
	read symbol1
	echo "$symbol1" > symbol.tmp
	if [ $(( $(grep "[a-zA-Z]" symbol.tmp| wc -m) - 1 )) -eq 1 ]
	then	
		break
	fi
done
...
# playing again after the first game
playAgain(){
